...
func init() {
	...
	syncCmd.Flags().StringVarP(&opts.inputFolder, "in", "i", "./", "Directory to look for raw image files")
	syncCmd.Flags().StringVarP(&opts.outputFolder, "out", "o", "./", "Directory to export jpgs to")
	syncCmd.Flags().StringVarP(&opts.command, "command", "c", "flatpak run --command=darktable-cli org.darktable.Darktable", "Darktable command or binary")
	syncCmd.Flags().StringVarP(&opts.extension, "extension", "e", ".ARW", "Extension of raw files")
}
...
func sync(cmd *cobra.Command, args []string) error {
	...
	raws := sidecars.FindFilesWithExt(opts.inputFolder, opts.extension)
	config := parseConfig()
	for _, raw := range raws {
		...
		if len(xmps) == 0 {
			...
			for _, xmp := range xmps {
				...
				// Export the RAW file
				params.XmpPath = xmp
				jpgFilename := sidecars.GetJpgFilename(xmp, opts.extension)
	...
	if config.DeleteMissing == "true" {
		...
		jpgs := sidecars.FindFilesWithExt(opts.outputFolder, ".jpg")
		deleteJpgs(sidecars.FindJpgsWithoutRaw(jpgs, opts.inputFolder, opts.outputFolder, opts.extension))
